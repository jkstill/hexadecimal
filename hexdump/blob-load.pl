...
$sql = q{insert into binary_test(name,my_raw) values(?, ?)};
$sth = $dbh->prepare($sql);
$sth->execute(q{RAW}, $hexRaw);

my $longRaw;
foreach my $i ( 1..9000 ) { $longRaw .= $raw }


$sql = q{insert into binary_test(name,my_long_raw) values(?, ?)};
$sth = $dbh->prepare($sql);
$sth->execute(q{LONG RAW}, $hexRaw);
